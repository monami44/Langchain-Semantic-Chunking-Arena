...
class GradientChunker:
    ...
    def __init__(self, embeddings: HuggingFaceEmbeddings):
        ...
        self.pubmed_dir = self.gradient_dir / "pubmed"
        self.arxiv_dir = self.gradient_dir / "arxiv"
    ...
    def split_text(self, text: str, source: str, file_name: str) -> List[str]:
        ...
        try:
            self.logger.info(f"Starting GradientChunker.split_text for {source} document: {unique_id}")
            ...
            self.logger.info(f"GradientChunker created {len(chunk_texts)} chunks for {source} document: {unique_id}")
            ...
            self.logger.error(f"Error in GradientChunker.split_text for {source} document {self.process_counter} ({file_name}): {e}")
...
print("GradientChunker class defined successfully with support for PubMed and arXiv sources")
