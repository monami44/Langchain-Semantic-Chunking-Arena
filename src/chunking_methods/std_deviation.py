# src/chunking_methods/std_deviation.py
...
class StdDeviationChunker:
    """
    Splits text into chunks based on standard deviation of sentence lengths.
    """
    def __init__(self, embeddings: HuggingFaceEmbeddings):
        print(f"Initializing StdDeviationChunker with embeddings: {embeddings}")
        ...
        # Create std_deviation directory and subdirectories if they don't exist
        self.std_deviation_dir = Path("chunks/std_deviation")
        self.pubmed_dir = self.std_deviation_dir / "pubmed"
        self.arxiv_dir = self.std_deviation_dir / "arxiv"
        self.std_deviation_dir.mkdir(parents=True, exist_ok=True)
        self.pubmed_dir.mkdir(exist_ok=True)
        self.arxiv_dir.mkdir(exist_ok=True)
        print(f"Standard Deviation directory structure created/verified: {self.std_deviation_dir}")
    ...
    def split_text(self, text: str, source: str, file_name: str) -> List[str]:
        ...
        try:
            self.logger.info(f"Starting StdDeviationChunker.split_text for {source} document: {unique_id}")
            ...
            self.logger.info(f"StdDeviationChunker created {len(chunk_texts)} chunks for {source} document: {unique_id}")
            ...
            self.logger.error(f"Error in StdDeviationChunker.split_text for {source} document {self.process_counter} ({file_name}): {e}")
...
print("StdDeviationChunker class defined successfully with support for PubMed and arXiv sources")
